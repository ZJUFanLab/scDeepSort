...
def normalize_weight(graph: dgl.DGLGraph, device: torch.device):
...
def get_id_2_gene(gene_statistics_path, mouse_data_path, tissue):
...
def get_id_2_label(cell_statistics_path, mouse_data_path, tissue):
...
def load_data(params):
    ...
    gene_gene_inter_path = mouse_data_path / 'mouse_ligand_receptor_pair.csv'
    ...
    id2gene = get_id_2_gene(gene_statistics_path, mouse_data_path, tissue)
    # generate cell label statistics file
    id2label = get_id_2_label(cell_statistics_path, mouse_data_path, tissue)
    ...
    for g1, g2 in gene_gene_inter:
        ...
        if g1_id is not None and g2_id is not None:
            ...
            gene_gene_edges.append([g2_id, g1_id])
    ...
    train_graph.add_nodes(num_genes, {'id': torch.arange(num_genes, dtype=torch.int32, device=device).unsqueeze(-1)})
    # add gene-gene edges
    train_graph.add_edges(gene_gene_edges[:, 0], gene_gene_edges[:, 1], {
        'weight': torch.tensor([1] * gene_gene_num, dtype=torch.float32, device=device).unsqueeze(-1)})
    train_graph.add_edges(gene_gene_edges[:, 1], gene_gene_edges[:, 0], {
        'weight': torch.tensor([1] * gene_gene_num, dtype=torch.float32, device=device).unsqueeze(-1)})
    for num in test:
        test_graph_dict[num] = dgl.DGLGraph()
        test_graph_dict[num].add_nodes(num_genes,
                                       {'id': torch.arange(num_genes, dtype=torch.int32, device=device).unsqueeze(-1)})
        test_graph_dict[num].add_edges(gene_gene_edges[:, 0], gene_gene_edges[:, 1], {
            'weight': torch.tensor([1] * gene_gene_num, dtype=torch.float32, device=device).unsqueeze(-1)})
        test_graph_dict[num].add_edges(gene_gene_edges[:, 1], gene_gene_edges[:, 0], {
            'weight': torch.tensor([1] * gene_gene_num, dtype=torch.float32, device=device).unsqueeze(-1)})
    ...
    for num in train + test:
        ...
        if num in train:
            train_num += len(df)
            train_graph.add_nodes(len(df),
                                  {'id': torch.tensor([-1] * len(df), device=device, dtype=torch.int32).unsqueeze(-1)})
            ...
            for n in test:  # training cell also in test graph
                test_graph_dict[n].add_nodes(len(df),
                                             {'id': torch.tensor([-1] * len(df), device=device,
            ...
            test_num += len(df)
            test_graph_dict[num].add_nodes(len(df),
                                           {'id': torch.tensor([-1] * len(df), device=device,
                                                               dtype=torch.int32).unsqueeze(-1)})
            # test_graph_dict[num].add_edges(cell_idx, gene_idx,
            #                                {'weight': torch.tensor(non_zeros, dtype=torch.float32,
            #                                                        device=device).unsqueeze(1)})
    ...
    # normalize weight & add self-loop
    normalize_weight(train_graph, device)
    ...
    for num in test:
        ...
        # normalize weight & add self-loop
        normalize_weight(test_graph_dict[num], device)
